...
class SchedulingController {
    async create(req, res){
        try { 
            ...
            res
                .status(500)
                .json({message: "Error while creating schedule!", error:error.message });
        }
    }
    async read(req, res){ 
        try{ const Scheduling = await SchedulingModel.find();
    ...
    async delete(req, res){
        try{
            ...
            return res.status(200).json({ message: "Scheduling deleted successfully"});
            } catch (error) {
                res
                    .status(500)
                    .json({ message: "Error while creating schedule!", error: error.message});
